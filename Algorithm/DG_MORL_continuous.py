"""DG_MORL_continuous Algorithm. Adapted from GPI-PD implementation
...
class DG_MORL_continuous(MOAgent, MOPolicy):
    """DG_MORL_continuous Algorithm. Adapted from GPI-PD implementation
    ...
    def get_agent_EU_(self, eval_env, eval_weights, eval_iterations, show_case=True):
        utilities = []
        for weight in eval_weights:
            disc_return, disc_vec_return, scalar_return, vec_return, _ = self.play_a_episode_(env=eval_env,
                                                                                              weight=weight,
                                                                                              demo_info=demo_info(
                                                                                                  horizon=0),
    ...
    def select_w_(self, eval_env, weights, mode="get_max"):
        ...
        for w in weights:
            demo_info, utility_threshold, max_disc_vec_return, demo_idx = self.weight_to_demo_(w=w, eval_env=eval_env,
                                                                                               eval_iterations=self.eval_iterations)
            u = self.get_agent_EU_(eval_env=eval_env,
    ...
    def jsmorl_train_iteration(self,
        ...
        while step < total_timesteps and demo_info.horizon >= 0:
            ...
            if self.global_step % eval_freq == 0:
                ...
                # u_thres_factor = min(0.9 + 0.1 * (self.global_step // 1.5e4), 0.98)
                u_thres_factor = 1
            ...
            if self.global_step % total_timesteps == 0:
                EU = self.get_agent_EU_(eval_weights=eval_weights, eval_env=eval_env,
                                        eval_iterations=self.eval_iterations, show_case=False)
...
class GPILSContinuousAction(DG_MORL_continuous):
...
if __name__ == '__main__':
    ...
    # This is the general version of training,
    # if the user want to do sensitivity experiment, they need to modify the corresponding parameters
    for seed in seeds:
        start_time = time.time()
        """ This part is for Experiment of MO-Hopper"""
        ...
        human_demos_1 = np.load("initial_demos//MO_Hopper//demos[1. 0.].npy", allow_pickle=True)
        human_demos_2 = np.load("initial_demos//MO_Hopper//demos[0.75 0.25].npy", allow_pickle=True)
        human_demos_3 = np.load("initial_demos//MO_Hopper//demos[0.5 0.5].npy", allow_pickle=True)
        human_demos_4 = np.load("initial_demos//MO_Hopper//demos[0.25 0.75].npy", allow_pickle=True)
        human_demos_5 = np.load("initial_demos//MO_Hopper//demos[0. 1.].npy", allow_pickle=True)
        ...
        # uncomment this to do quantity sensitivity evaluation
        # human_demos = random.choices(human_demos, k=3)
        agent.jsmorl_train(demos=human_demos, eval_env=eval_env, total_timesteps=15e4, timesteps_per_iter=1.5e4,
                           title="MO_Hopper_seed_" + str(seed),
                           eval_freq=100,
                           roll_back_step=100)
        end_time = time.time()
        ...
        print(f"Run with seed {seed} for {elapsed_time_in_hours:.2f} hours")
